...
**Structured exception:** total amnesia has a failure mode of its own — oscillation. A debugger that can't see which hypotheses already failed will retry the same two plausible implementations on alternating attempts forever. The fix isn't conversation, it's a **capped, text-only diagnostic receipt**: root cause + failed operations, no transcript, no reasoning trace. Enough to avoid the loop, too small to pollute. Give the receipt a schema, not prose: `failed_assertion` (the exact failing check), `hypotheses_tested[]` (what's disproven, so retries don't oscillate), capped at ~20 lines — this schema is this blueprint's own design, not drawn from the CodeDelegator citation below. Anything outside the schema is dropped at the gate — an unstructured receipt becomes the bleed vector it was meant to prevent.
